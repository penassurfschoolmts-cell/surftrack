...
function AdminDashboard({ user }) {
  ...
  // ── INSTRUCTORS ──
  async function createInstructor(name, email, phone, notes, studentId) {
    await db.createInstructor(name, email, phone, notes, studentId);
    await loadInstructors();
  }
    ...
    <LangContext.Provider value={lang}>
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          {page === "instructors"   && <InstructorsPage instructors={instructors} students={students} createInstructor={safe(createInstructor)} updateInstructor={safe(updateInstructor)} deleteInstructor={safe(deleteInstructor)} logInstructorSession={safe(logInstructorSession)} todayStr={todayStr} />}
...
// INSTRUCTORS PAGE
// ─────────────────────────────────────────────
function InstructorsPage({ instructors, students = [], createInstructor, updateInstructor, deleteInstructor, logInstructorSession, todayStr }) {
  const [showNew, setShowNew]       = useState(false);
  const [editing, setEditing]       = useState(null);
  const [selected, setSelected]     = useState(null);
  const [showLogSession, setShowLogSession] = useState(false);
  const [form, setForm]             = useState({ name: "", email: "", phone: "", notes: "", studentId: "" });
  ...
  function openNew() { setForm({ name: "", email: "", phone: "", notes: "", studentId: "" }); setEditing(null); setShowNew(true); }
  function openEdit(ins) { setForm({ name: ins.name, email: ins.email, phone: ins.phone || "", notes: ins.notes || "", studentId: ins.student_id || "" }); setEditing(ins); setShowNew(true); }
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        ...
        await createInstructor(form.name.trim(), form.email.trim(), form.phone, form.notes, form.studentId || null);
        ...
        <div className="modal-overlay" onClick={() => setShowNew(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <div style={{ padding: "0 24px 24px" }}>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>Link to Employee <span style={{ color: "var(--border)" }}>— optional</span></div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 6 }}>Links this instructor to a staff member for combined attendance + teaching tracking.</div>
                <select className="input" value={form.studentId} onChange={e => setForm(p => ({ ...p, studentId: e.target.value }))}>
                  <option value="">— Guest / external instructor (no link) —</option>
