...
const T = {
  ...
  pt: {
    ...
    exportDateRange: "Período",
    exportSelected: "Selecionado",
    exportPresets: {
      thisMonth:   "Este mês",
...
function playPing(allowed) {
  try {
    ...
    if (allowed) {
      // Success: two short rising tones
      ...
      // Denied: one low descending tone
...
function getWeekStart() {
  // Week starts Sunday 00:00 local time. Return as YYYY-MM-DD date string
  // to avoid any UTC/timezone conversion issues.
  const now = new Date();
...
function isNewWeek(ws) {
  if (!ws) return true;
  // Compare as date strings — both are YYYY-MM-DD or ISO timestamps starting with YYYY-MM-DD
  const wsDate = ws.slice(0, 10);
  return wsDate < getWeekStart();
}
...
const db = {
  ...
  async createStudent(fields) {
    const { data, error } = await supabase
      .from("students")
      .insert({ name: fields.name, email: fields.email, phone: fields.phone || "", avatar: initials(fields.name), notes: fields.notes || "", role: fields.role || "student" })
      .select()
      .single();
    if (error) throw error;
    return data;
  ...
  async updateStudentDetails(id, fields) {
    const { error } = await supabase.from("students").update({ name: fields.name, email: fields.email, phone: fields.phone, notes: fields.notes }).eq("id", id);
    if (error) throw error;
  },
  async updateStudentRole(id, role) {
    await supabase.from("students").update({ role }).eq("id", id);
  },

  // ── MEMBERSHIPS ───────────────────────────
  async upsertMembership(studentId, tierId) {
    console.log("[db] upsertMembership", { studentId, tierId });
    // active=false until payment confirmed; valid_until set on activation
    const { data, error } = await supabase
      .from("memberships")
      .upsert({ student_id: studentId, tier_id: tierId, active_week_entries: 0, week_start: getWeekStart(), active: false, valid_until: null, paid_at: null }, { onConflict: "student_id" })
      .select();
  ...
  async activateMembership(studentId) {
    const validUntil = getLastDayOfMonth();
    const { error } = await supabase.from("memberships")
      .update({ active: true, valid_until: validUntil, paid_at: new Date().toISOString() })
      .eq("student_id", studentId);
    if (error) throw error;
  },

  async deactivateMembership(studentId) {
    const { error } = await supabase.from("memberships")
      .update({ active: false })
      .eq("student_id", studentId);
    if (error) throw error;
  ...
  // Called on load — deactivate memberships where valid_until < today
  async deactivateExpiredMemberships(studentIds) {
    ...
    const { error } = await supabase.from("memberships")
      ...
      .eq("active", true);
    if (error) console.warn("[deactivateExpiredMemberships]", error);
  },

  async activatePunchCard(studentId) {
    const { error } = await supabase.from("punch_cards")
      .update({ active: true, paid_at: new Date().toISOString() })
      .eq("student_id", studentId);
    if (error) throw error;
  },

  async activateCampEnrollment(studentId, campId) {
    const { error } = await supabase.from("camp_enrollments")
      .update({ active: true, paid_at: new Date().toISOString() })
      .eq("student_id", studentId).eq("camp_id", campId);
    if (error) throw error;
  },
  ...
  async upsertPunchCard(studentId, balance) {
    ...
    const { data, error } = await supabase
      .from("punch_cards")
      .upsert({ student_id: studentId, balance, active: false, paid_at: null }, { onConflict: "student_id" })
      .select();
  ...
  async enrollStudent(studentId, campId, schedule) {
    const { error } = await supabase.from("camp_enrollments").insert({ student_id: studentId, camp_id: campId, schedule, active: false, paid_at: null });
    if (error) throw error;
  },
  ...
  async listCampStaff(campId) {
    const { data, error } = await supabase.from("camp_staff")
      .select("*, instructors(*), students(*)")
      .eq("camp_id", campId);
    if (error) throw error;
    return data || [];
  },
  async addCampStaff(campId, { instructorId, studentId, role, dateFrom, dateTo }) {
    const { error } = await supabase.from("camp_staff").insert({
      camp_id: campId,
  ...
  async removeCampStaff(id) {
    const { error } = await supabase.from("camp_staff").delete().eq("id", id);
    if (error) throw error;
  },
  ...
  async getCampEnrollmentForStudent(studentId, date) {
    // Returns all active + paid camp enrollments for a student on a given date
    const { data, error } = await supabase
  ...
  async listPrivateInstructors() {
    const { data, error } = await supabase
      .from("instructors")
      .select("id, name, email, phone, avatar, available_for_private")
      .eq("available_for_private", true)
      .order("name");
    if (error) throw error;
    return data || [];
  },
  async updateInstructorPrivateAvailability(id, available) {
    const { error } = await supabase.from("instructors").update({ available_for_private: available }).eq("id", id);
    if (error) throw error;
  },
  // ── STUDENT REQUESTS ─────────────────────────
  async listStudentRequests() {
    const { data, error } = await supabase
      ...
      .select("*, students(name, email, avatar)")
      .eq("status", "pending")
  ...
  async updateRequestStatus(id, status) {
    const { error } = await supabase.from("student_requests").update({ status }).eq("id", id);
    if (error) throw error;
  },
  async createInstructor(name, email, phone, notes, studentId) {
    const { error } = await supabase.from("instructors")
      .insert({ name, email, phone: phone || "", notes: notes || "", avatar: (name.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2)), student_id: studentId || null });
    if (error) throw error;
  },
  ...
  async getInstructorByStudentId(studentId) {
    const { data, error } = await supabase.from("instructors").select("*").eq("student_id", studentId).limit(1);
    if (error) return null;
    return data?.[0] || null;
  },
  async getTodayInstructorReservation(instructorId, date) {
    const { data, error } = await supabase.from("reservations")
      .select("*").eq("instructor_id", instructorId).eq("session_date", date)
      .neq("status", "cancelled").limit(1);
    if (error) return null;
    return data?.[0] || null;
  },
  // Get instructor assigned to a specific date+slot (group class assignment)
  async getSlotInstructor(date, slot) {
    const { data } = await supabase.from("reservations")
      .select("instructor_id").eq("session_date", date).eq("session_slot", slot)
      .not("instructor_id", "is", null).limit(1);
    if (!data?.[0]?.instructor_id) return null;
    const { data: ins } = await supabase.from("instructors").select("*").eq("id", data[0].instructor_id).limit(1);
    return ins?.[0] || null;
  },
  // Check if an instructor session was already logged today for this slot (avoid double-logging)
  async instructorSessionExists(instructorId, date, slot) {
    const { data } = await supabase.from("instructor_sessions")
      .select("id").eq("instructor_id", instructorId).eq("session_date", date).eq("session_slot", slot).limit(1);
  ...
  // ── LESSONS ───────────────────────────────
  async logLesson(studentId, lessonType, label, className, punchCost, ts = null) {
    const created_at = ts || new Date().toISOString();
  ...
  // ── EXPORT ────────────────────────────────
  async exportEntryLog(from, to) {
    let q = supabase.from("entry_log_detailed").select("*");
  ...
  async exportRevenue(from, to) {
    let q = supabase.from("revenue_summary").select("*");
    if (from) q = q.gte("created_at", from);
    if (to)   q = q.lte("created_at", to + "T23:59:59.999Z");
    const { data } = await q.order("created_at", { ascending: false });
    return data || [];
...
function normalizeStudent(row) {
  ...
  return {
    ...
    membership: m ? { tierId: m.tier_id, activeWeekEntries: m.active_week_entries, weekStart: m.week_start, active: m.active !== false, validUntil: m.valid_until || null, paidAt: m.paid_at || null } : null,
    punchCard: pc ? { balance: pc.balance, active: pc.active !== false, paidAt: pc.paid_at || null } : null,
...
function AdminDashboard({ user }) {
  ...
  const [campStaff, setCampStaff] = useState([]);
  ...
  function getThisWeekSaturday() {
    const now = new Date();
    const sat = new Date(now.getFullYear(), now.getMonth(), now.getDate() - now.getDay() + 6);
  ...
  const loadStudentRequests = useCallback(async () => {
    try { setStudentRequests(await db.listStudentRequests()); }
    catch (e) { console.warn("[student requests load]", e); }
  }, []);

  const load = useCallback(async (silent = false) => {
    try {
      if (!silent) setInitialLoading(true);
      const fetchedStudents = await db.fetchStudents();
      const today = new Date().toISOString().slice(0, 10);
      const weekStart = getWeekStart();

      // Deactivate memberships whose valid_until has passed
      const expired = fetchedStudents.filter(s =>
        s.membership?.active && s.membership?.validUntil && s.membership.validUntil < today
      );
      if (expired.length > 0) {
        await db.deactivateExpiredMemberships(expired.map(s => s.id));
      }

      // Batch-reset any members whose weekStart is before this week — lazy reset would miss them
      const stale = fetchedStudents.filter(s =>
        s.membership && s.membership.weekStart && s.membership.weekStart.slice(0, 10) < weekStart
      );
      if (stale.length > 0) {
        await Promise.all(stale.map(s =>
          supabase.from("memberships").update({ active_week_entries: 0, week_start: weekStart }).eq("student_id", s.id)
        ));
      }

      // Refetch if anything changed, otherwise use what we have
      if (expired.length > 0 || stale.length > 0) {
        setStudents(await db.fetchStudents());
      } else {
  ...
  useEffect(() => { load(); loadReservations(); loadCamps(); loadInstructors(); loadStudentRequests(); }, [load, loadReservations, loadCamps, loadInstructors, loadStudentRequests]);
  ...
  async function processEntry(studentId, isGuest = false, guestName = "") {
    ...
    // Employees get free entry — no charge, no membership/punch deduction
    if (student.role === "employee") {
      await db.logEntry(studentId, "employee", "Employee entry", false, "");
      await load(true);
      return { allowed: true, message: "Welcome! 👋", detail: "Employee entry — no charge." };
    }

    await db.resetWeekIfNeeded(student);

    if (student.membership && student.membership.active) {
    ...
    } else if (student.membership && !student.membership.active) {
      // Membership exists but payment not confirmed — fall through to punch card or deny
    }

    if (student.punchCard?.active && student.punchCard?.balance > 0) {
    ...
    // Explain why if they have products but none are active
    const hasInactiveM = student.membership && !student.membership.active;
    const hasInactiveP = student.punchCard && !student.punchCard.active;
    if (hasInactiveM || hasInactiveP) {
      await db.logEntry(studentId, "denied", "Payment pending", isGuest, guestName);
      await load(true);
      return { allowed: false, message: "Payment Pending ⚠", detail: "Membership or punch card not yet activated. Please confirm payment." };
    }
  ...
  async function logLesson(studentId, lessonType, className = "") {
    ...
    // Employees and camp students attend for free — no punch or membership deduction
    if (student?.role === "employee" || student?.role === "camp") {
      await db.logEntry(studentId, student.role, `${lt.label}${className ? `: ${className}` : ""}`, false, "");
      await db.logLesson(studentId, lessonType, lt.label, className, 0);
      await load(true);
      return { ok: true, method: student.role };
    }
    ...
    await db.logEntry(studentId, "punchcard", `${lt.label}${className ? `: ${className}` : ""}`, false, "");
    await db.logLesson(studentId, lessonType, lt.label, className, lt.punchCost);
    await load(true);
    return { ok: true, method: "punch_card" };
  }

  // Retroactive lesson — deducts normally but logs with a past timestamp
  async function logRetroactiveLesson(studentId, lessonType, dateStr, slot) {
    const student = students.find(s => s.id === studentId);
    const lt = lessonTypes.find(l => l.id === lessonType);
    if (!student || !lt) return { ok: false, msg: "Student or lesson type not found." };
    const ts = `${dateStr}T${slot === "7am" ? "07:00" : slot === "9:30am" ? "09:30" : slot === "10am" ? "10:00" : slot === "2pm" ? "14:00" : "10:00"}:00.000Z`;
    const note = `${lt.label} — retroactive (${dateStr} ${slot})`;

    if (student.role === "employee") {
      await db.logEntry(studentId, "employee", note, false, "", ts);
      await db.logLesson(studentId, lessonType, lt.label, slot, 0, ts);
      await load(true);
      return { ok: true, method: "employee" };
    }
    const tier = student.membership ? getTier(student.membership.tierId, tiers) : null;
    const weeklyRemaining = tier ? Math.max(0, (tier.entriesPerWeek || 0) - (student.membership.activeWeekEntries || 0)) : 0;
    if (student.membership?.active && weeklyRemaining > 0) {
      const newCount = (student.membership.activeWeekEntries || 0) + 1;
      await db.incrementWeekEntry(studentId, newCount);
      await db.logEntry(studentId, "membership", note, false, "", ts);
      await db.logLesson(studentId, lessonType, lt.label, slot, 0, ts);
      await load(true);
      return { ok: true, method: "membership" };
    }
    if (student.punchCard?.active && student.punchCard.balance >= lt.punchCost) {
      const newBal = student.punchCard.balance - lt.punchCost;
      await db.updatePunchBalance(studentId, newBal);
      await db.logPunchHistory(studentId, -lt.punchCost, note);
      await db.logEntry(studentId, "punchcard", note, false, "", ts);
      await db.logLesson(studentId, lessonType, lt.label, slot, lt.punchCost, ts);
      await load(true);
      return { ok: true, method: "punch_card" };
    }
    // No active plan — log as manual override with a warning
    await db.logEntry(studentId, "membership", `${note} [manual — no active plan]`, false, "", ts);
    await db.logLesson(studentId, lessonType, lt.label, slot, 0, ts);
    await load(true);
  ...
  async function processEntryWithReservation(studentId, isGuest = false, guestName = "") {
    const student = students.find(s => s.id === studentId);
    // Camp-only students: skip all punch/membership checks — they're handled by the camp path below
    if (student?.role === "camp") {
      ...
      if (campEnrs.length > 0) {
        ...
        return { allowed: true, message: "Camp Check-in! 🏕", detail: `${camp.name} · ${scheduleLabel}` };
      }
      return { allowed: false, message: "No Active Camp", detail: "This student has no active camp enrollment for today." };
    }

    if (student?.role === "employee") {
      const today = todayStr();
      // Check if this employee is a linked instructor with a class today
      const instructor = await db.getInstructorByStudentId(studentId);
      let detail = "Employee entry — no charge.";
      let note = "Employee entry";
      if (instructor) {
        const teachingRes = await db.getTodayInstructorReservation(instructor.id, today);
        if (teachingRes) {
          detail = `Teaching ${teachingRes.session_slot} class today 🏄`;
          note = `Employee entry — teaching ${teachingRes.session_slot}`;
          // Also log an instructor session automatically
          await db.logInstructorSession(instructor.id, today, teachingRes.session_slot, "Auto-logged at check-in");
          await loadInstructors();
        }
      }
      await db.logEntry(studentId, "employee", note, false, "");
      await load(true);
      return { allowed: true, message: "Welcome! 👋", detail };
    }
    ...
    if (res) {
      ...
      // Auto-log instructor session on student check-in
      // Private: instructor on the individual reservation
      // Group: instructor assigned to anyone in that slot
      async function maybeLogInstructor() {
        try {
          const isPrivate = res.lesson_type === "private";
          let instructor = null;
          if (isPrivate && res.instructor_id) {
            const { data } = await supabase.from("instructors").select("*").eq("id", res.instructor_id).limit(1);
            instructor = data?.[0] || null;
          } else if (!isPrivate) {
            instructor = await db.getSlotInstructor(today, res.session_slot);
          }
          if (instructor) {
            const alreadyLogged = await db.instructorSessionExists(instructor.id, today, res.session_slot);
            if (!alreadyLogged) {
              await db.logInstructorSession(instructor.id, today, res.session_slot, "Auto-logged at student check-in");
              await loadInstructors();
            }
          }
        } catch(e) { console.warn("[instructor auto-log]", e); }
      }
      ...
      if (student.punchCard && student.punchCard.active) {
      ...
      // Has inactive products — payment pending
      const hasInactiveM = student.membership && !student.membership.active;
      const hasInactiveP = student.punchCard && !student.punchCard.active;
      if (hasInactiveM || hasInactiveP) {
        return { allowed: false, message: "Payment Pending ⚠", detail: "Membership or punch card not yet activated. Please confirm payment." };
  ...
  // ── INSTRUCTORS ──
  async function createInstructor(name, email, phone, notes, studentId, makeEmployee) {
    let resolvedStudentId = studentId || null;
    // If makeEmployee is true, create a student record with role=employee first
    if (makeEmployee) {
      const row = await db.createStudent({ name, email: email || "", phone: phone || "", notes: notes || "", role: "employee" });
      resolvedStudentId = row.id;
      await load(true);
    }
    await db.createInstructor(name, email, phone, notes, resolvedStudentId);
    await loadInstructors();
  }
  ...
  // Creates an employee student record for an existing instructor and links them
  async function createEmployeeForInstructor(instructorId, name, email, phone) {
    const row = await db.createStudent({ name, email: email || "", phone: phone || "", notes: "", role: "employee" });
    await db.updateInstructor(instructorId, { student_id: row.id });
    await load(true);
    await loadInstructors();
    return row.id;
  }
  ...
  async function addCampStaff(campId, fields) {
    await db.addCampStaff(campId, fields);
    await loadCamps();
  }
  async function removeCampStaff(id) {
    await db.removeCampStaff(id);
    await loadCamps();
  }
  ...
  async function adjustWeekSessions(studentId, newCount) {
  ...
  async function toggleInstructorPrivate(instructorId, current) {
    await db.updateInstructorPrivateAvailability(instructorId, !current);
    await loadInstructors();
  }

  async function activateMembership(studentId) {
    await db.activateMembership(studentId);
    await load(true);
  }
  async function deactivateMembership(studentId) {
    await db.deactivateMembership(studentId);
    await load(true);
  }
  async function activatePunchCard(studentId) {
    await db.activatePunchCard(studentId);
    await load(true);
  }
  async function activateCampEnrollment(studentId, campId) {
    await db.activateCampEnrollment(studentId, campId);
    await loadCamps();
    await load(true);
  }
  ...
  async function updateStudentDetails(studentId, fields) {
    await db.updateStudentDetails(studentId, fields);
    await load(true);
  }
    ...
    <LangContext.Provider value={lang}>
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          {page === "dashboard"     && <Dashboard students={students} reservations={reservations} setPage={setPage} setModal={setModal} setSelectedId={setSelectedId} studentRequests={studentRequests} approveRequest={safe(approveRequest)} rejectRequest={safe(rejectRequest)} />}
          {page === "students"      && <StudentsPage students={filtered} search={search} setSearch={setSearch} setSelectedId={setSelectedId} setModal={setModal} studentRequests={studentRequests} />}
          ...
          {page === "camps"         && <CampsPage students={students} camps={camps} campEnrollments={campEnrollments} campAttendance={campAttendance} campStaff={campStaff} instructors={instructors} createCamp={safe(createCamp)} deleteCamp={safe(deleteCamp)} enrollStudent={safe(enrollStudentInCamp)} unenrollStudent={safe(unenrollStudentFromCamp)} checkInCamper={safe(checkInCamper)} activateCampEnrollment={safe(activateCampEnrollment)} addCampStaff={safe(addCampStaff)} removeCampStaff={safe(removeCampStaff)} todayStr={todayStr} />}
          {page === "instructors"   && <InstructorsPage instructors={instructors} students={students} createInstructor={safe(createInstructor)} updateInstructor={safe(updateInstructor)} deleteInstructor={safe(deleteInstructor)} logInstructorSession={safe(logInstructorSession)} createEmployeeForInstructor={safe(createEmployeeForInstructor)} toggleInstructorPrivate={safe(toggleInstructorPrivate)} todayStr={todayStr} />}
      ...
      {modal === "studentDetail" && selectedId && (
        <StudentDetailModal
          ...
          processEntry={safe(processEntry)} logLesson={safe(logLesson)} logRetroactiveLesson={safe(logRetroactiveLesson)}
          addPunches={safe(addPunches)} setMembership={safe(setMembership)} removeMembership={safe(removeMembership)}
          correctEntry={safe(correctEntry)} adjustPunchBalance={safe(adjustPunchBalance)} adjustWeekSessions={safe(adjustWeekSessions)}
          activateMembership={safe(activateMembership)} deactivateMembership={safe(deactivateMembership)}
          activatePunchCard={safe(activatePunchCard)}
          updateStudentDetails={safe(updateStudentDetails)}
...
var SLOT_ORDER = ["7am", "9:30am", "10am", "2pm", "Private", "Camp", "Employee", "After Hours"];
// ─────────────────────────────────────────────
var FILTER_OPTIONS = [
  { id: "all",        label: "All" },
            ...
            <div key={s.id} style={{ marginBottom: 12, cursor: "pointer", paddingBottom: 12, borderBottom: "1px solid var(--border)" }}
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap8">
                  ...
                  {!tier && !s.punchCard && <span className={`badge ${s.role === "employee" ? "badge-gold" : "badge-gray"}`}>{s.role === "employee" ? "Employee" : "No plan"}</span>}
                </div>
              </div>
              {tier && (
...
// DASHBOARD
// ─────────────────────────────────────────────
function Dashboard({ students, reservations = [], setPage, setModal, setSelectedId, studentRequests = [], approveRequest, rejectRequest }) {
  const t       = useLang();
  ...
  function classifySlot(entry) {
    ...
    if (entry.method === "employee") return "Employee";
    if (entry.method === "camp") return "Camp";
    if (note.includes("9:30")) return "9:30am";
    if (note.includes("7am")) return "7am";
    if (note.includes("10am")) return "10am";
    if (note.includes("2pm")) return "2pm";
    if (note.includes("private")) return "Private";
    const d = new Date(entry.ts);
    const day = d.getDay();
    const mins = d.getHours() * 60 + d.getMinutes();
    if (day === 0 || day === 6) return "9:30am";
    if (mins < 8 * 60 + 30) return "7am";
    if (mins < 12 * 60 + 30) return "10am";
    if (mins < 15 * 60 + 30) return "2pm";
    return "After Hours";
  }
  ...
  allEntries.slice(0, 60).forEach(e => {
    ...
    const slot = slotOverrides[e.id] || classifySlot(e);
  ...
  // Build Sun–Sat for current week using local date arithmetic (avoids UTC offset bug)
  function getWeekDays() {
    const now = new Date();
    const sunDate = now.getDate() - now.getDay();
    return Array.from({ length: 7 }, (_, i) => {
      const d = new Date(now.getFullYear(), now.getMonth(), sunDate + i);
        ...
        <div className="card" style={{ marginBottom: 20, borderLeft: "3px solid var(--gold)" }}>
          <div className="row" style={{ justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div className="row gap8" style={{ alignItems: "center" }}>
              <div className="card-title" style={{ marginBottom: 0 }}>⏳ Student Requests</div>
              <span className="badge badge-gold">{studentRequests.length}</span>
            </div>
          </div>
          {studentRequests.map(req => {
            const p = req.payload || {};
            const typeLabel = req.type === "punch_topup" ? "Punch top-up" : req.type === "membership_change" ? "Membership change" : "Private lesson";
            const detail = req.type === "punch_topup" ? p.label || `${p.punches || "?"} punches`
              : req.type === "membership_change" ? p.tier_label || p.id || "?"
              : `${p.instructor_name || "?"} · ${p.date || "?"} ${p.slot || ""}`;
            ...
            return (
              <div key={req.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid var(--border)" }}>
                <div className="avatar" style={{ width: 32, height: 32, fontSize: 11, flexShrink: 0 }}>{student?.avatar || "?"}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: 13 }}>{student?.name || "Unknown"}</div>
                  <div style={{ fontSize: 11, color: "var(--muted)" }}>{typeLabel} · {detail}</div>
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  <button className="btn btn-primary btn-sm" style={{ fontSize: 11, padding: "3px 10px" }}
                    onClick={() => { setSelectedId(req.student_id); setModal("studentDetail"); approveRequest(req.id); }}>
                    ✓ View
                  </button>
                  <button className="btn btn-ghost btn-sm" style={{ fontSize: 11, padding: "3px 10px", color: "var(--red)" }}
                  ...
                  <div key={slot} style={{ marginBottom: 10 }}>
                    ...
                    {slots[slot].map((e, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 6px", borderRadius: 6, marginBottom: 2, background: e.method === "denied" ? "rgba(217,79,92,.06)" : "var(--surface2)" }}>
                        ...
                        <select
                          style={{ fontSize: 10, padding: "1px 4px", border: "1px solid var(--border)", borderRadius: 4, background: slotOverrides[e.id] ? "rgba(42,143,189,.1)" : "transparent", color: "var(--muted)", cursor: "pointer", outline: "none" }}
                          value={slotOverrides[e.id] || slot}
                          onChange={ev => setSlotOverrides(prev => ({ ...prev, [e.id]: ev.target.value }))}
...
// RESERVATIONS PAGE
// ─────────────────────────────────────────────
var WEEKDAY_SLOTS  = ["7am", "10am", "2pm"];
var WEEKEND_SLOTS  = ["9:30am"];
var ALL_SLOTS      = ["7am", "9:30am", "10am", "2pm"];
...
function ReservationsPage({ students, reservations, lessonTypes, instructors = [], createReservation, cancelReservation, chargeNoShow, updateReservationInstructor, todayStr, getThisWeekSunday }) {
  ...
  // Build Sun–Sat using local date arithmetic (avoids UTC offset shifting the week by 1 day)
  function getWeekDays() {
    const sundayStr = getThisWeekSunday();
                  ...
                  return (
                    <td key={date} style={{ padding: "6px", verticalAlign: "top", borderBottom: "1px solid var(--border)", background: date === today ? "rgba(42,143,189,0.04)" : "transparent", minWidth: 110 }}>
                      {slotRes.map(r => {
                        ...
                        const isPrivate = r.lesson_type === "private";
                        return (
                          <div key={r.id} style={{ background: "var(--surface2)", borderRadius: 7, padding: "5px 8px", marginBottom: 4, fontSize: 11, border: "1px solid var(--border)" }}>
                            ...
                            {/* Instructor assignment — only on individual card for private lessons */}
                            {isPrivate && instructors.length > 0 && (
                      ...
                      {/* Slot-level instructor for group classes */}
                      {instructors.length > 0 && (() => {
                        const hasGroupRes = slotRes.some(r => r.lesson_type !== "private");
                        if (!hasGroupRes) return null;
                        // Find instructor assigned to any group res in this slot
                        const groupRes = slotRes.filter(r => r.lesson_type !== "private");
                        const assignedId = groupRes.find(r => r.instructor_id)?.instructor_id || "";
                        const assignedName = assignedId ? instructors.find(i => i.id === assignedId)?.name : null;
                        return (
                          <div style={{ marginBottom: 4, padding: "3px 6px", background: assignedId ? "rgba(42,143,189,.08)" : "transparent", borderRadius: 5, border: assignedId ? "1px solid var(--border)" : "none" }}>
                            {assignedId
                              ? <div style={{ fontSize: 10, color: "var(--accent)", fontWeight: 600 }}>🏄 {assignedName}
                                  <button style={{ marginLeft: 6, background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 10 }} onClick={() => groupRes.forEach(r => updateReservationInstructor(r.id, null))}>✕</button>
                                </div>
                              : <select
                                  style={{ fontSize: 10, padding: "2px 4px", width: "100%", background: "transparent", border: "1px dashed var(--border)", borderRadius: 4, color: "var(--muted)" }}
                                  value=""
                                  onChange={e => { if (e.target.value) groupRes.forEach(r => updateReservationInstructor(r.id, e.target.value)); }}
                                >
...
// INSTRUCTORS PAGE
// ─────────────────────────────────────────────
function InstructorsPage({ instructors, students = [], createInstructor, updateInstructor, deleteInstructor, logInstructorSession, createEmployeeForInstructor, toggleInstructorPrivate, todayStr }) {
  const [showNew, setShowNew]       = useState(false);
  const [editing, setEditing]       = useState(null);
  const [selected, setSelected]     = useState(null);
  const [showLogSession, setShowLogSession] = useState(false);
  const [form, setForm]             = useState({ name: "", email: "", phone: "", notes: "", studentId: "", isStaff: false });
  const [logForm, setLogForm]       = useState({ date: todayStr(), slot: "10am", notes: "" });
  const [busy, setBusy]             = useState(false);
  const [msg, setMsg]               = useState(null);

  const employees = students.filter(s => s.role === "employee");
  ...
  function openNew() { setForm({ name: "", email: "", phone: "", notes: "", studentId: "", isStaff: false }); setEditing(null); setShowNew(true); }
  function openEdit(ins) {
    setForm({ name: ins.name, email: ins.email, phone: ins.phone || "", notes: ins.notes || "", studentId: ins.student_id || "", isStaff: false });
    setEditing(ins);
    setShowNew(true);
  }
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        await updateInstructor(editing.id, { name: form.name.trim(), email: form.email.trim(), phone: form.phone, notes: form.notes, avatar: form.name.trim().split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2), student_id: form.studentId || null });
        setMsg({ text: "Instructor updated!", ok: true });
      } else {
        // Pass isStaff flag — action will create employee record if true
        await createInstructor(form.name.trim(), form.email.trim(), form.phone, form.notes, form.studentId || null, form.isStaff);
        setMsg({ text: form.isStaff ? "Instructor & employee record created!" : "Instructor added!", ok: true });
      }
      setShowNew(false);
  ...
  async function handleCreateEmployeeRecord(ins) {
    if (!window.confirm(`Create an employee/student record for ${ins.name}? This will add them to the Students list with role = Employee.`)) return;
    setBusy(true);
    try {
      await createEmployeeForInstructor(ins.id, ins.name, ins.email, ins.phone);
      setMsg({ text: "Employee record created and linked!", ok: true });
    } catch(e) { setMsg({ text: e.message, ok: false }); }
    finally { setBusy(false); }
  }
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <div style={{ display: "flex", gap: 6, marginTop: 2, flexWrap: "wrap" }}>
                          {ins.student_id && (() => { const emp = employees.find(e => e.id === ins.student_id); return emp ? <span className="badge badge-gold" style={{ fontSize: 10 }}>👤 {emp.name}</span> : null; })()}
                          {ins.available_for_private && <span className="badge badge-green" style={{ fontSize: 10 }}>🔑 Private</span>}
                        </div>
                      </div>
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
                    ...
                    <button
                      ...
                      style={{ fontSize: 11, color: ins.available_for_private ? "var(--green)" : "var(--muted)" }}
                      onClick={e => { e.stopPropagation(); toggleInstructorPrivate && toggleInstructorPrivate(ins.id, ins.available_for_private); }}
                      title={ins.available_for_private ? "Remove from private lesson availability" : "Mark as available for private lessons"}
                    >
                      {ins.available_for_private ? "🔑 Private: On" : "🔑 Private: Off"}
                    </button>
                    {!ins.student_id && (
                      <button className="btn btn-ghost btn-sm" style={{ fontSize: 11, color: "var(--accent)" }} onClick={e => { e.stopPropagation(); handleCreateEmployeeRecord(ins); }} title="Create an employee/student record for this instructor">
                        + Staff record
                      </button>
                    )}
        ...
        <div className="modal-overlay" onClick={() => setShowNew(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <div style={{ padding: "0 24px 24px" }}>
              ...
              <div style={{ marginBottom: 20 }}>
                {/* NEW instructor: show "is staff" checkbox OR link to existing employee */}
                {!editing && (<>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 10 }}>Staff / Attendance</div>
                  <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: form.isStaff ? 0 : 10, padding: "10px 12px", background: form.isStaff ? "rgba(42,143,189,.08)" : "var(--surface2)", borderRadius: 8, border: form.isStaff ? "1px solid var(--accent)" : "1px solid var(--border)" }}>
                    <input type="checkbox" checked={form.isStaff} onChange={e => setForm(p => ({ ...p, isStaff: e.target.checked, studentId: "" }))} />
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>This instructor is a staff member</div>
                      <div style={{ fontSize: 11, color: "var(--muted)" }}>Creates an employee record so their attendance is tracked</div>
                    </div>
                  </label>
                  {!form.isStaff && (<>
                    <div style={{ fontSize: 11, color: "var(--muted)", margin: "10px 0 6px" }}>Or link to an existing employee:</div>
                    <select className="input" value={form.studentId} onChange={e => setForm(p => ({ ...p, studentId: e.target.value }))}>
                      <option value="">— Guest / no employee link —</option>
                      {employees.filter(e => !instructors.find(i => i.student_id === e.id)).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                  </>)}
                </>)}
                {/* EDIT instructor: show link dropdown */}
                {editing && (<>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 8 }}>Employee Link</div>
                  <select className="input" value={form.studentId} onChange={e => setForm(p => ({ ...p, studentId: e.target.value }))}>
                    <option value="">— Guest / no employee link —</option>
...
// CAMPS PAGE
// ─────────────────────────────────────────────
function CampsPage({ students, camps, campEnrollments, campAttendance, campStaff = [], instructors = [], createCamp, deleteCamp, enrollStudent, unenrollStudent, checkInCamper, activateCampEnrollment, addCampStaff, removeCampStaff, todayStr }) {
  ...
  const [staffForm, setStaffForm] = useState({ type: "instructor", id: "", role: "instructor", dateFrom: "", dateTo: "" });
  ...
  const campStaffForCamp = camp ? campStaff.filter(s => s.camp_id === camp.id) : [];

  // Employees (students with role=employee) not already in camp staff
  const employees = students.filter(s => s.role === "employee");
  const staffedEmployeeIds = campStaffForCamp.filter(s => s.student_id).map(s => s.student_id);
  const staffedInstructorIds = campStaffForCamp.filter(s => s.instructor_id).map(s => s.instructor_id);
  const availableEmployees = employees.filter(e => !staffedEmployeeIds.includes(e.id));
  const availableInstructors = instructors.filter(i => !staffedInstructorIds.includes(i.id));

  async function handleAddStaff() {
    if (!camp || !staffForm.id) return;
    setBusy(true);
    try {
      await addCampStaff(camp.id, {
        instructorId: staffForm.type === "instructor" ? staffForm.id : null,
        studentId: staffForm.type === "employee" ? staffForm.id : null,
        role: staffForm.role,
        dateFrom: staffForm.dateFrom || null,
        dateTo: staffForm.dateTo || null,
      });
      setShowAddStaff(false);
      setStaffForm({ type: "instructor", id: "", role: "instructor", dateFrom: "", dateTo: "" });
    } catch(e) { setMsg({ text: e.message, ok: false }); }
    finally { setBusy(false); }
  }
        ...
        {camp ? (
          <div className="card">
                ...
                <div key={e.student_id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid var(--border)" }}>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 600 }}>{s?.name || "?"}</span>
                      <span style={{ fontSize: 10, fontWeight: 700, color: isActive ? "var(--green)" : "var(--red)" }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {!isActive && (
                      <button className="btn btn-primary btn-sm" style={{ fontSize: 10, padding: "2px 8px" }} disabled={busy}
                        onClick={async () => { setBusy(true); await activateCampEnrollment(e.student_id, camp.id); setBusy(false); }}>
                        ✓ Confirm
                      </button>
                    )}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 20, marginBottom: 10 }}>
              <div style={{ fontSize: 13, fontWeight: 600 }}>Staff / Instructors ({campStaffForCamp.length})</div>
              <button className="btn btn-ghost btn-sm" onClick={() => setShowAddStaff(true)}>+ Add Staff</button>
            </div>
            {campStaffForCamp.length === 0 && <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 8 }}>No staff assigned to this camp yet.</div>}
            {campStaffForCamp.map(sf => {
              const name = sf.instructors?.name || sf.students?.name || "?";
              const badge = sf.instructors ? "Instructor" : "Employee";
              const dateRange = (sf.date_from || sf.date_to)
                ? `${sf.date_from || "start"} → ${sf.date_to || "end"}`
                : "Full camp";
              return (
                <div key={sf.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid var(--border)" }}>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 600 }}>{name}</span>
                      <span style={{ fontSize: 10, background: "var(--surface2)", padding: "1px 6px", borderRadius: 4, color: "var(--muted)" }}>{badge}</span>
                      {sf.role && sf.role !== "instructor" && sf.role !== "employee" && (
                        <span style={{ fontSize: 10, color: "var(--accent)" }}>{sf.role}</span>
                      )}
                    </div>
                    <div style={{ fontSize: 10, color: "var(--muted)" }}>{dateRange}</div>
                  </div>
                  <button className="btn btn-ghost btn-sm" style={{ fontSize: 10, color: "var(--red)", padding: "2px 8px" }} disabled={busy}
                    onClick={async () => { setBusy(true); await removeCampStaff(sf.id); setBusy(false); }}>Remove</button>
                </div>
              );
            })}
          </div>
      ...
      {showAddStaff && camp && (
        <div className="modal-overlay" onClick={() => setShowAddStaff(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 380 }}>
            <div className="modal-header">
              <div className="modal-title">Add Staff — {camp.name}</div>
              <button className="modal-close" onClick={() => setShowAddStaff(false)}>✕</button>
            </div>
            <div style={{ padding: "0 24px 24px" }}>
              {/* Type toggle */}
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                {[["instructor","🏄 Instructor"],["employee","👤 Employee"]].map(([val, lbl]) => (
                  <div key={val} className={`opt-card ${staffForm.type === val ? "selected" : ""}`}
                    style={{ flex: 1, padding: "8px 12px", textAlign: "center" }}
                    onClick={() => setStaffForm(p => ({ ...p, type: val, id: "" }))}>
                    <div className="opt-card-label" style={{ fontSize: 12 }}>{lbl}</div>
                  </div>
                ))}
              </div>
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 6 }}>
                  {staffForm.type === "instructor" ? "Instructor" : "Employee"}
                </div>
                <select className="input" value={staffForm.id} onChange={e => setStaffForm(p => ({ ...p, id: e.target.value }))}>
                  <option value="">— Select —</option>
                  {staffForm.type === "instructor"
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 6 }}>Role / Title (optional)</div>
                <input className="input" placeholder="e.g. Lead Instructor, Assistant…" value={staffForm.role}
                  onChange={e => setStaffForm(p => ({ ...p, role: e.target.value }))} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                <div>
                  <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 6 }}>From (optional)</div>
                  <input className="input" type="date" value={staffForm.dateFrom}
                    min={camp.start_date} max={camp.end_date}
                    onChange={e => setStaffForm(p => ({ ...p, dateFrom: e.target.value }))} />
                </div>
                <div>
                  <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 6 }}>To (optional)</div>
                  <input className="input" type="date" value={staffForm.dateTo}
                    min={camp.start_date} max={camp.end_date}
                    onChange={e => setStaffForm(p => ({ ...p, dateTo: e.target.value }))} />
                </div>
              </div>
              <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 14 }}>
                Leave dates blank to assign for the full camp duration.
              </div>
              <button className="btn btn-primary btn-full" disabled={!staffForm.id || busy} onClick={handleAddStaff}>
                {busy ? "Adding…" : "Add to Camp"}
              </button>
            </div>
          </div>
        </div>
...
// STUDENTS PAGE
// ─────────────────────────────────────────────
function StudentsPage({ students, search, setSearch, setSelectedId, setModal, studentRequests = [] }) {
  const t      = useLang();
  const { tiers } = useConfig();
      ...
      <div className="card"><div className="table-wrap"><table>
        ...
        <tbody>
          {students.map((s, i) => {
            ...
            const hasPendingRequest = studentRequests.some(r => r.student_id === s.id && r.status === "pending");
            return (
              <tr key={s.id}>
                <td><div className="name-cell"><div className="avatar" style={{ background: colors[i % 3], color: "#0a0e12" }}>{s.avatar}</div><div><div className="bold">{s.name}{hasPendingRequest && <span style={{ marginLeft: 6, fontSize: 10, background: "var(--gold)", color: "#fff", padding: "1px 6px", borderRadius: 4, verticalAlign: "middle" }}>⏳ Request</span>}</div><div className="small text-muted">{s.email}</div></div></div></td>
...
function EntryPage({ students, processEntry }) {
  ...
  const [deviceId, setDeviceId] = useState(null);     // currently selected device id
  ...
  // Enumerate cameras — prefer USB (non-environment) when available
  async function loadDevices() {
    try {
      // Must request permission first or labels will be empty
      const perm = await navigator.mediaDevices.getUserMedia({ video: true }).catch(() => null);
      if (perm) perm.getTracks().forEach(t => t.stop());
      const all = await navigator.mediaDevices.enumerateDevices();
      const videoDevices = all.filter(d => d.kind === "videoinput");
      setDevices(videoDevices);
      if (videoDevices.length > 0 && !deviceId) {
        // Prefer a device that isn't labelled as front-facing
        const preferred = videoDevices.find(d => /(usb|external|back|rear|environment)/i.test(d.label))
          || videoDevices[videoDevices.length - 1]; // last device is often external USB
        setDeviceId(preferred.deviceId);
      }
    } catch(e) { console.warn("[devices]", e); }
  }

  async function startCamera(overrideDeviceId) {
    setCamErr("");
    stopCamera();
    try {
      const targetId = overrideDeviceId || deviceId;
      // If we have a specific device, use it; otherwise fall back to environment facing mode
      const videoConstraints = targetId
      ...
      // If exact deviceId fails (e.g. device unplugged), fall back to any camera
      if (overrideDeviceId || deviceId) {
        try {
  ...
  async function scanFrame() {
    ...
    try {
      ...
      if (!decoded) {
        // jsQR fallback — downsample to max 640px wide for speed + reliability
        ...
        if (canvas) {
          ...
          // "attemptBoth" handles inverted QR codes (dark-on-light AND light-on-dark)
  ...
  function switchCamera(newDeviceId) {
    ...
    if (mode === "camera") startCamera(newDeviceId);
  }

  useEffect(() => {
    if (mode === "camera") { loadDevices().then(() => startCamera()); }
    else stopCamera();
    return stopCamera;
  }, [mode]);

  // Re-start camera when deviceId changes (if already in camera mode)
  useEffect(() => {
...
function ExportPage() {
  ...
  function getDateRange() {
    ...
    switch (preset) {
      case "thisMonth": {
        ...
        return { from, to: todayStr, label: today.toLocaleString("en-GB", { month: "long", year: "numeric" }) };
      }
      case "lastMonth": {
        const from = new Date(y, m - 1, 1).toISOString().slice(0, 10);
        const to   = new Date(y, m, 0).toISOString().slice(0, 10);
        const d    = new Date(y, m - 1, 1);
        return { from, to, label: d.toLocaleString("en-GB", { month: "long", year: "numeric" }) };
      }
      case "thisYear": {
        return { from: `${y}-01-01`, to: todayStr, label: `${y}` };
      }
      case "lastYear": {
      ...
      case "last3months": {
        ...
        return { from, to: todayStr, label: t.exportPresets.last3months };
      }
      case "last6months": {
        const from = new Date(y, m - 5, 1).toISOString().slice(0, 10);
        return { from, to: todayStr, label: t.exportPresets.last6months };
      }
      case "allTime":
        return { from: null, to: null, label: t.exportPresets.allTime };
      case "custom":
        return { from: customFrom || null, to: customTo || null, label: `${customFrom || t.exportFrom} → ${customTo || todayStr}` };
      default:
  ...
  const PRESETS = [
    { id: "thisMonth",   label: t.exportPresets.thisMonth },
    { id: "lastMonth",   label: t.exportPresets.lastMonth },
    { id: "last3months", label: t.exportPresets.last3months },
    { id: "last6months", label: t.exportPresets.last6months },
    { id: "thisYear",    label: t.exportPresets.thisYear },
    { id: "lastYear",    label: t.exportPresets.lastYear },
    { id: "allTime",     label: t.exportPresets.allTime },
    { id: "custom",      label: t.exportPresets.custom },
  ];

  async function exportEntries() {
    const { from, to, label } = getDateRange();
    setBusy("entries");
    const rows = await db.exportEntryLog(from, to);
    downloadCSV(rows, `surftrack-entries-${label.replace(/\s+/g,"-")}.csv`);
    setMsg(`✓ ${rows.length} entry records downloaded (${label}).`);
    setBusy(""); setTimeout(() => setMsg(""), 4000);
  }

  async function exportRevenue() {
    const { from, to, label } = getDateRange();
    setBusy("revenue");
    const rows = await db.exportRevenue(from, to);
    downloadCSV(rows, `surftrack-revenue-${label.replace(/\s+/g,"-")}.csv`);
    setMsg(`✓ ${rows.length} revenue records downloaded (${label}).`);
    setBusy(""); setTimeout(() => setMsg(""), 4000);
  }
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 20, maxWidth: 700 }}>
        <div className="card-title">{t.exportDateRange}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: preset === "custom" ? 16 : 0 }}>
          {PRESETS.map(p => (
            <button
              key={p.id}
        ...
        {preset === "custom" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 14 }}>
            <div>
              <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>{t.exportFrom}</div>
              <input className="input" type="date" value={customFrom} max={todayStr} onChange={e => setCustomFrom(e.target.value)} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>{t.exportTo}</div>
              <input className="input" type="date" value={customTo} max={todayStr} onChange={e => setCustomTo(e.target.value)} />
            </div>
          </div>
        )}
        {preset !== "custom" && (
          <div style={{ marginTop: 10, fontSize: 12, color: "var(--accent)", fontWeight: 600 }}>
            {t.exportSelected}: {label}
          </div>
        )}
      </div>
      ...
      <div className="grid2" style={{ gap: 20, maxWidth: 700 }}>
        <div className="card">
          ...
          <button className="btn btn-primary btn-full" disabled={busy === "entries"} onClick={exportEntries}>
            {busy === "entries" ? t.downloading : `↓ Download — ${label}`}
          </button>
        </div>
        <div className="card">
          <div className="card-title">{t.revenueSummaryTitle}</div>
          <div className="small text-muted mb16">{t.revenueSummaryDesc}</div>
          <button className="btn btn-primary btn-full" disabled={busy === "revenue"} onClick={exportRevenue}>
            {busy === "revenue" ? t.downloading : `↓ Download — ${label}`}
          </button>
        </div>
      </div>
...
// STUDENT DETAIL MODAL
// ─────────────────────────────────────────────
function RetroLogForm({ student, lessonTypes, logRetroactiveLesson, flash, busy, setBusy }) {
  const [retroDate, setRetroDate] = useState("");
  const [retroSlot, setRetroSlot] = useState("10am");
  const [retroType, setRetroType] = useState(lessonTypes[0]?.id || "group");
  ...
  async function handleLog() {
    if (!retroDate) return;
    setBusy(true); setRetroResult(null);
    const r = await logRetroactiveLesson(student.id, retroType, retroDate, retroSlot);
    setRetroResult(r);
    if (r.ok) flash(`✓ Session logged (${retroDate} ${retroSlot}) — deducted from ${r.method}.`);
    setBusy(false);
  }
    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
        <div>
          ...
          <input className="input" type="date" max={todayStr} value={retroDate} onChange={e => setRetroDate(e.target.value)} />
        </div>
        <div>
          <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>Slot</div>
          <select className="input" value={retroSlot} onChange={e => setRetroSlot(e.target.value)}>
            {["7am","9:30am","10am","2pm"].map(s => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
      </div>
      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>Lesson Type</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {lessonTypes.map(lt => (
            <div key={lt.id} className={`opt-card ${retroType === lt.id ? "selected" : ""}`} style={{ padding: "6px 12px" }} onClick={() => setRetroType(lt.id)}>
              <div className="opt-card-label" style={{ fontSize: 12 }}>{lt.label}</div>
            </div>
          ))}
        </div>
      </div>
      {retroResult && !retroResult.ok && (
        <div className="notice-warn" style={{ marginBottom: 10, fontSize: 12 }}>✗ {retroResult.msg}</div>
      )}
      <button
        className="btn btn-primary btn-sm"
        disabled={busy || !retroDate}
        onClick={handleLog}
      >
        ↩ Log Past Session
      </button>
      {!retroDate && <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 6 }}>Select a date to continue.</div>}
    </div>
  );
}

function StudentDetailModal({ student, onClose, processEntry, logLesson, logRetroactiveLesson, addPunches, setMembership, removeMembership, correctEntry, adjustPunchBalance, adjustWeekSessions, activateMembership, deactivateMembership, activatePunchCard, deleteStudent, sendWelcomeEmail, updateStudentDetails }) {
  const t = useLang();
  const { lessonTypes, groupClasses, tiers, cardOptions } = useConfig();
  ...
  const [detailForm, setDetailForm] = useState({ name: "", email: "", phone: "", notes: "" });
    ...
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {student.role === "employee" && <span className="badge badge-gold">Employee</span>}
              </div>
              <div className="small text-muted">{student.email} · {t.joinedDate(fmt(student.joinDate))}</div>
            </div>
          </div>
        ...
        {tab === "overview" && (<>
          {/* Role toggle */}
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[["student","🏄 Student"],["employee","👤 Employee"],["camp","🏕 Camp"]].map(([val, lbl]) => (
              <div key={val} className={`opt-card ${student.role === val ? "selected" : ""}`} style={{ flex: 1, padding: "8px 12px", textAlign: "center" }}
                onClick={async () => { await db.updateStudentRole(student.id, val); await load(true); flash(val === "employee" ? "Marked as Employee" : val === "camp" ? "Marked as Camp student" : "Marked as Student"); }}>
                <div className="opt-card-label" style={{ fontSize: 12 }}>{lbl}</div>
              </div>
            ))}
          </div>
          {student.role === "employee" && (
            <div className="notice" style={{ marginBottom: 16 }}>
              👤 This person is an <strong>employee</strong>. Entry is free, not charged against membership or punch card.
            </div>
          )}
          {student.role === "camp" && (
            <div className="notice" style={{ marginBottom: 16 }}>
              🏕 Camp-only student. Entry is only allowed via active camp enrollment — no punch card or membership required.
            </div>
          )}
          {student.role !== "employee" && student.role !== "camp" && (<>
          <div className="grid2">
            <div>
          ...
          {student.notes && !editingDetails && <div className="notice mt16">📝 {student.notes}</div>}
          {/* Contact details — read or edit */}
          <div className="separator" />
          {!editingDetails ? (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.8 }}>
                {student.phone && <div>📞 {student.phone}</div>}
                <div>✉ {student.email || <span style={{ color: "var(--border)" }}>No email</span>}</div>
              </div>
              <button className="btn btn-ghost btn-sm" style={{ fontSize: 11 }} onClick={() => { setDetailForm({ name: student.name, email: student.email || "", phone: student.phone || "", notes: student.notes || "" }); setEditingDetails(true); }}>
                ✏ Edit
              </button>
            </div>
          ) : (
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--accent)", marginBottom: 12, textTransform: "uppercase", letterSpacing: ".05em" }}>Edit Details</div>
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>Full Name</div>
                <input className="input" value={detailForm.name} onChange={e => setDetailForm(p => ({ ...p, name: e.target.value }))} />
              </div>
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>Email</div>
                <input className="input" type="email" value={detailForm.email} onChange={e => setDetailForm(p => ({ ...p, email: e.target.value }))} />
              </div>
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>Phone</div>
                <input className="input" value={detailForm.phone} onChange={e => setDetailForm(p => ({ ...p, phone: e.target.value }))} placeholder="+351 912 345 678" />
              </div>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>Notes</div>
                <textarea className="input" rows={2} value={detailForm.notes} onChange={e => setDetailForm(p => ({ ...p, notes: e.target.value }))} />
              </div>
              <div className="row gap8">
                <button className="btn btn-primary btn-sm" disabled={busy || !detailForm.name.trim()} onClick={async () => { setBusy(true); try { await updateStudentDetails(student.id, detailForm); setEditingDetails(false); flash("Details updated!"); } catch(e) { flash("Error: " + e.message); } finally { setBusy(false); } }}>
        ...
        {tab === "entry" && (<>
          ...
          {isGuest && <div className="field"><label>{t.guestName}</label><input value={guestName} onChange={e => setGuestName(e.target.value)} placeholder={t.guestNamePlaceholder} /></div>}
          <button className="btn btn-primary btn-full" disabled={busy} onClick={async () => { setBusy(true); const res = await processEntry(student.id, isGuest, guestName); setResult(res); playPing(res.allowed); setBusy(false); }}>{busy ? t.processing : t.processEntryBtn}</button>
        ...
        {tab === "lessons" && (<>
          {!student.punchCard && !student.membership && student.role !== "employee" && student.role !== "camp" && <div className="notice-warn">{t.studentNeedsPunchCard}</div>}
          ...
          <button className="btn btn-primary btn-full" disabled={(!student.punchCard && !student.membership && student.role !== "employee" && student.role !== "camp") || busy} onClick={async () => { setBusy(true); const r = await logLesson(student.id, lessonType, lessonType === "group" ? cls : ""); flash(r.ok ? t.lessonRecorded : `✗ ${r.msg}`); setBusy(false); }}>{busy ? t.saving : t.logLessonBtn}</button>
        </>)}

        {tab === "billing" && (<>
          {/* ── Payment Activation Panel ── */}
          {(student.membership || student.punchCard) && (() => {
            const mActive = student.membership?.active;
            const mUntil  = student.membership?.validUntil;
            const pcActive = student.punchCard?.active;
            const anyPending = (student.membership && !mActive) || (student.punchCard && !pcActive);
            return (
              <div style={{ marginBottom: 20 }}>
                <div className="section-label" style={{ marginBottom: 8 }}>Payment & Activation</div>
                {student.membership && (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: mActive ? "rgba(34,197,94,.08)" : "rgba(239,68,68,.06)", borderRadius: 8, border: `1px solid ${mActive ? "var(--green)" : "var(--red)"}`, marginBottom: 8 }}>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: mActive ? "var(--green)" : "var(--red)" }}>
                        {mActive ? "✓ Membership Active" : "⚠ Membership — Payment Pending"}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>
                        {getTier(student.membership.tierId, tiers)?.label}
                        {mActive && mUntil ? ` · valid until ${mUntil}` : ""}
                      </div>
                    </div>
                    {mActive
                      ? <button className="btn btn-ghost btn-sm" style={{ fontSize: 11, color: "var(--muted)" }} disabled={busy} onClick={async () => { setBusy(true); await deactivateMembership(student.id); flash("Membership deactivated."); setBusy(false); }}>Deactivate</button>
                      : <button className="btn btn-primary btn-sm" disabled={busy} onClick={async () => { setBusy(true); await activateMembership(student.id); flash("✓ Membership activated — valid until end of month."); setBusy(false); }}>✓ Confirm Payment</button>
                    }
                  </div>
                )}
                {student.punchCard && (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: pcActive ? "rgba(34,197,94,.08)" : "rgba(239,68,68,.06)", borderRadius: 8, border: `1px solid ${pcActive ? "var(--green)" : "var(--red)"}`, marginBottom: 8 }}>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: pcActive ? "var(--green)" : "var(--red)" }}>
                        {pcActive ? "✓ Punch Card Active" : "⚠ Punch Card — Payment Pending"}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>
                        {student.punchCard.balance} punches remaining
                      </div>
                    </div>
                    {!pcActive && (
                      <button className="btn btn-primary btn-sm" disabled={busy} onClick={async () => { setBusy(true); await activatePunchCard(student.id); flash("✓ Punch card activated."); setBusy(false); }}>✓ Confirm Payment</button>
                    )}
                  </div>
                )}
                {anyPending && (
                  <div className="notice-warn" style={{ fontSize: 11, marginTop: 4 }}>
                    Entry will be denied until payment is confirmed.
                  </div>
                )}
          ...
          {student.membership && (() => {
            ...
            return (
              <div style={{ marginTop: 20 }}>
                <div className="separator" />
                <div className="section-label" style={{ marginBottom: 4 }}>Weekly Session Override</div>
                <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 12 }}>
                  Current week: <strong style={{ color: "var(--text)" }}>{used} / {limit}</strong> sessions used
                  {used > limit && <span style={{ color: "var(--gold)", marginLeft: 8 }}>⚠ Over limit</span>}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <button
                    className="btn btn-ghost btn-sm"
                    disabled={busy || used <= 0}
                    onClick={async () => { setBusy(true); await adjustWeekSessions(student.id, Math.max(0, used - 1)); flash("Session removed."); setBusy(false); }}
                  >− Remove session</button>
                  <span style={{ fontSize: 13, fontWeight: 700, minWidth: 60, textAlign: "center", color: used > limit ? "var(--red)" : "var(--text)" }}>{used} used</span>
                  <button
                    className="btn btn-ghost btn-sm"
                    style={{ color: "var(--accent)" }}
                    disabled={busy}
                    onClick={async () => { setBusy(true); await adjustWeekSessions(student.id, used + 1); flash("Extra session granted."); setBusy(false); }}
                  >+ Grant session</button>
                </div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 8 }}>
                  Week resets automatically on Sunday regardless of this override.
                </div>
              </div>
            );
          })()}
        ...
        {tab === "corrections" && (<>
          ...
          <div className="section-label" style={{ marginBottom: 10 }}>Log a Past Session</div>
          <div className="notice" style={{ marginBottom: 12, fontSize: 12 }}>
            Backdates an entry to the selected date. Deducts from membership or punch card as normal.
          </div>
          <RetroLogForm student={student} lessonTypes={lessonTypes} logRetroactiveLesson={logRetroactiveLesson} flash={flash} busy={busy} setBusy={setBusy} />
...
function AddStudentModal({ onClose, onAdd }) {
  ...
  const needsPlan = role === "student";

  async function submit() {
    if (!name.trim() || !email.trim()) { setErr(t.nameEmailRequired); return; }
    setBusy(true);
    try { await onAdd({ name: name.trim(), email: email.trim(), phone, notes, role }, needsPlan ? (hasM ? tierId : null) : null, needsPlan ? (hasP ? cardId : null) : null); }
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {[["student","🏄 Student"],["employee","👤 Employee"],["camp","🏕 Camp"]].map(([val, lbl]) => (
            <div key={val} className={`opt-card ${role === val ? "selected" : ""}`} style={{ flex: 1, padding: "10px 14px", textAlign: "center" }} onClick={() => setRole(val)}>
              <div className="opt-card-label" style={{ fontSize: 13 }}>{lbl}</div>
            </div>
          ))}
        </div>
        {isCamp && <div className="notice" style={{ marginBottom: 12, fontSize: 12 }}>🏕 Camp-only student — no membership or punch card needed. Enroll them in a camp from the Camps page after creating the account.</div>}
        <div className="grid2">
