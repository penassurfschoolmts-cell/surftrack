...
function AdminDashboard({ user }) {
  ...
  async function sendWelcomeEmail(student, tierId, cardId) {
    // Build QR data URL using the same qrcode lib used in EntryPage
    const QRCode = (await import("https://cdn.jsdelivr.net/npm/qrcode@1.5.3/build/qrcode.min.js")).default
      .catch ? null : null;
    // Generate QR as data URL via canvas
    const canvas = document.createElement("canvas");
    await new Promise((res, rej) => {
      if (window.QRCode) {
        window.QRCode.toCanvas(canvas, student.id, { width: 200 }, e => e ? rej(e) : res());
      } else {
        // fallback: load qrcode dynamically
        const s = document.createElement("script");
        s.src = "https://cdn.jsdelivr.net/npm/qrcode@1.5.3/build/qrcode.min.js";
        s.onload = () => window.QRCode.toCanvas(canvas, student.id, { width: 200 }, e => e ? rej(e) : res());
        s.onerror = rej;
