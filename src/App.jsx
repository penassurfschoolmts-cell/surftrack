...
// QR EXPORT HELPERS
// ─────────────────────────────────────────────
function buildQRSvgString(clientId, size = 300) {
  const N    = 21;
  const seed = clientId.split("").reduce((a, c) => a + c.charCodeAt(0) * 31, 0);
  const rng  = i => { const x = Math.sin(seed + i * 127.1) * 43758.5453; return x - Math.floor(x); };
  const grid = Array.from({ length: N * N }, (_, i) => {
  ...
  const cs = size / N;
  const rects = grid.map((on, i) => on
    ? `<rect x="${(i % N) * cs + 0.3}" y="${Math.floor(i / N) * cs + 0.3}" width="${cs - 0.6}" height="${cs - 0.6}" fill="#1a1a2e" rx="1"/>`
    : "").join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}"><rect width="${size}" height="${size}" fill="#ffffff" rx="4"/>${rects}</svg>`;
}
...
async function downloadQRPng(student) {
  ...
  const svgQR  = buildQRSvgString(student.id, size);
  const qrDataUrl = await svgToPngDataUrl(svgQR, size, size);
  const canvas = document.createElement("canvas");
  ...
  const r = 16;
  ctx.beginPath();
  ctx.roundRect(pad - 8, pad - 8, size + 16, size + 16, r);
  ctx.fill();
  // QR image
...
async function downloadWalletCard(student) {
  ...
  // QR code (right side)
  const qrSize = 240;
  const qrSvg  = buildQRSvgString(student.id, qrSize);
  const qrDataUrl = await svgToPngDataUrl(qrSvg, qrSize, qrSize);
  const qrImg = new Image();
  await new Promise(res => { qrImg.onload = res; qrImg.src = qrDataUrl; });
  ...
  ctx.fill();
  ctx.drawImage(qrImg, W - qrSize - 50, H / 2 - qrSize / 2, qrSize, qrSize);
