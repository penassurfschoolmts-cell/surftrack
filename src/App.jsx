...
function AdminDashboard({ user }) {
  ...
  function getThisWeekSunday() {
    const d = new Date(getThisWeekMonday());
  ...
  const loadReservations = useCallback(async () => {
    try { setReservations(await db.listReservations(getThisWeekMonday(), getThisWeekSunday())); }
  ...
  useEffect(() => { load(); loadReservations(); }, [load, loadReservations]);
  ...
  // Called by scanner: check for a reservation then process entry
  async function processEntryWithReservation(studentId, isGuest = false, guestName = "") {
    ...
    if (res) {
      // Use the lesson type stored on the reservation
      const savedLessonType = res.lesson_type;
      const savedLt = lessonTypes.find(l => l.id === savedLessonType) || lessonTypes[0];
      const student = students.find(s => s.id === studentId);
      if (!student) return { allowed: false, message: "Student not found.", detail: "" };
      await db.resetWeekIfNeeded(student);
      // Charge via membership first, then punch card
      if (student.membership) {
        const tier = getTier(student.membership.tierId, tiers);
        const used = isNewWeek(student.membership.weekStart) ? 0 : student.membership.activeWeekEntries;
        const limit = tier?.entriesPerWeek || 0;
    ...
    // No reservation — fall through to normal entry
    ...
    <LangContext.Provider value={lang}>
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          {page === "reservations"  && <ReservationsPage students={students} reservations={reservations} lessonTypes={lessonTypes} createReservation={safe(createReservation)} cancelReservation={safe(cancelReservation)} chargeNoShow={safe(chargeNoShow)} todayStr={todayStr} getThisWeekMonday={getThisWeekMonday} />}
...
function Dashboard({ students, reservations = [], setPage, setModal, setSelectedId }) {
  ...
  const SLOTS   = ["7am", "10am", "2pm"];
  const todayRes = reservations.filter(r => r.session_date === today && r.status !== "cancelled");
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="row" style={{ justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div className="card-title" style={{ marginBottom: 0 }}>📅 Today's Sessions</div>
          <button className="btn btn-ghost btn-sm" onClick={() => setPage("reservations")}>View all →</button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {SLOTS.map(slot => {
            const slotRes = todayRes.filter(r => r.session_slot === slot);
            return (
              <div key={slot} style={{ background: "var(--surface2)", borderRadius: 10, padding: 12 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--accent)", marginBottom: 8 }}>{slot}</div>
                {slotRes.length === 0
                  ? <div style={{ fontSize: 11, color: "var(--muted)" }}>No reservations</div>
                  : slotRes.map(r => {
                    ...
                    return (
                      <div key={r.id} style={{ fontSize: 12, padding: "4px 0", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span>{s?.name || "?"}</span>
                        <span style={{ fontSize: 10, color: statusColor }}>{r.status}</span>
                      </div>
                    );
                  })
                }
...
function ReservationsPage({ students, reservations, lessonTypes, createReservation, cancelReservation, chargeNoShow, todayStr, getThisWeekMonday }) {
  const t = useLang();
            ...
            {SLOTS.map(slot => (
              <tr key={slot}>
                ...
                {days.map(date => {
                  ...
                  return (
                    <td key={date} style={{ padding: "6px", verticalAlign: "top", borderBottom: "1px solid var(--border)", background: date === today ? "rgba(0,229,255,0.03)" : "transparent", minWidth: 110 }}>
                      {slotRes.map(r => {
                        const s = students.find(st => st.id === r.student_id);
                        const lt = lessonTypes.find(l => l.id === r.lesson_type);
                        return (
                          <div key={r.id} style={{ background: "var(--surface2)", borderRadius: 7, padding: "5px 8px", marginBottom: 4, fontSize: 11 }}>
