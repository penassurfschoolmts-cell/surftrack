...
// ─────────────────────────────────────────────
// QR CODE — real scannable QR using qrcode library from CDN
// ─────────────────────────────────────────────
let _qrcodeLib = null;
async function loadQRLib() {
  ...
  return new Promise((resolve) => {
    ...
    s.onload = () => { _qrcodeLib = window.QRCode; resolve(window.QRCode); };
    s.onerror = () => resolve(null);
    document.head.appendChild(s);
  });
}

function QRCode({ clientId, size = 180, fg = "#1a1a2e", bg = "#ffffff" }) {
  const canvasRef = useRef(null);
  useEffect(() => {
    let cancelled = false;
    loadQRLib().then(QRLib => {
      if (cancelled || !QRLib || !canvasRef.current) return;
      QRLib.toCanvas(canvasRef.current, clientId, {
        width: size, margin: 2,
...
async function buildQRCanvas(clientId, size) {
  ...
  if (!QRLib) return canvas;
  await QRLib.toCanvas(canvas, clientId, { width: size, margin: 2, color: { dark: "#1a1a2e", light: "#ffffff" } });
  return canvas;
}
...
function EntryPage({ students, processEntry }) {
  ...
  async function loadJsQR() {
    ...
    return new Promise((resolve) => {
      ...
      s.onload = () => { jsqrRef.current = window.jsQR; resolve(window.jsQR); };
      s.onerror = () => resolve(null);
      document.head.appendChild(s);
    });
  }
  ...
  async function startCamera() {
    ...
    try {
      ...
      // Set up BarcodeDetector or jsQR fallback
  ...
  async function scanFrame() {
    ...
    try {
      ...
      if (decoded) {
        // decoded value is the student UUID — find and process immediately
