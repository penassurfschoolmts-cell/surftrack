...
function AdminDashboard({ user }) {
  ...
  async function logLesson(studentId, lessonType, className = "") {
    ...
    // Employees attend for free — no punch or membership deduction
    if (student?.role === "employee") {
      await db.logEntry(studentId, "employee", `${lt.label}${className ? `: ${className}` : ""}`, false, "");
...
// CAMPS PAGE
// ─────────────────────────────────────────────
function CampsPage({ students, camps, campEnrollments, campAttendance, createCamp, deleteCamp, enrollStudent, unenrollStudent, checkInCamper, activateCampEnrollment, todayStr }) {
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        {tab === "overview" && (<>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[["student","🏄 Student"],["employee","👤 Employee"]].map(([val, lbl]) => (
              <div key={val} className={`opt-card ${student.role === val ? "selected" : ""}`} style={{ flex: 1, padding: "8px 12px", textAlign: "center" }}
                onClick={async () => { await db.updateStudentRole(student.id, val); await load(true); flash(val === "employee" ? "Marked as Employee" : "Marked as Student"); }}>
          ...
          {student.role === "employee" && (
            <div className="notice" style={{ marginBottom: 16 }}>
              👤 This person is an <strong>employee</strong>. Entry is free, not charged against membership or punch card. Attendance is tracked separately in Recent Entries.
            </div>
          )}
          {student.role !== "employee" && (<>
          <div className="grid2">
        ...
        {tab === "lessons" && (<>
          {!student.punchCard && !student.membership && student.role !== "employee" && <div className="notice-warn">{t.studentNeedsPunchCard}</div>}
          ...
          <button className="btn btn-primary btn-full" disabled={(!student.punchCard && !student.membership && student.role !== "employee") || busy} onClick={async () => { setBusy(true); const r = await logLesson(student.id, lessonType, lessonType === "group" ? cls : ""); flash(r.ok ? t.lessonRecorded : `✗ ${r.msg}`); setBusy(false); }}>{busy ? t.saving : t.logLessonBtn}</button>
        </>)}
...
function AddStudentModal({ onClose, onAdd }) {
  ...
  async function submit() {
    ...
    setBusy(true);
    try { await onAdd({ name: name.trim(), email: email.trim(), phone, notes, role }, isEmployee ? null : (hasM ? tierId : null), isEmployee ? null : (hasP ? cardId : null)); }
