...
var SLOT_ORDER = ["7am", "9:30am", "10am", "2pm", "Private", "Camp", "After Hours"];
// ─────────────────────────────────────────────
var FILTER_OPTIONS = [
  { id: "all",        label: "All" },
...
function Dashboard({ students, reservations = [], setPage, setModal, setSelectedId }) {
  ...
  function classifySlot(entry) {
    ...
    // Camp entries always their own bucket
    if (entry.method === "camp") return "Camp";
    // If the note already has an explicit slot from the reservations flow, use it
    ...
    // Fall back to time-of-entry rules
    const d = new Date(entry.ts);
    const day = d.getDay(); // 0=Sun, 6=Sat
    const h = d.getHours(), m = d.getMinutes();
    const mins = h * 60 + m;
    if (day === 0 || day === 6) return "9:30am"; // weekends: only one class
    // Weekday windows
    if (mins < 8 * 60 + 30) return "7am";           // before 8:30 → 7am class
    if (mins < 12 * 60 + 30) return "10am";          // 8:30–12:30 → 10am class
    if (mins < 15 * 60 + 30) return "2pm";           // 12:30–15:30 → 2pm class
    return "After Hours";
  }

  // Group by date then by slot — respect any manual overrides
  const entryDays = {};
  ...
  const [slotOverrides, setSlotOverrides] = useState({}); // { [entryId]: slotLabel }
