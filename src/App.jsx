...
function EntryPage({ students, processEntry }) {
  ...
  async function handleEntry(studentId) {
    ...
    setBusy(false);
    // If we're in camera mode, stay there — clear result after 2.5s and resume scanning
      ...
      <div className="grid2" style={{ gap: 20, maxWidth: 860 }}>
        <div>
          <div className="tabs" style={{ marginBottom: 16 }}>
            <div className={`tab ${mode === "camera" ? "active" : ""}`} onClick={() => { setModeSync("camera"); setResult(null); }}>{t.cameraScan}</div>
            <div className={`tab ${mode === "search" ? "active" : ""}`} onClick={() => { setModeSync("search"); setResult(null); }}>{t.searchStudent}</div>
          </div>
          {mode === "camera" && (
